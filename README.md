...
2. https://www.netlify.com 가입 → **Add new site** → **Import an existing project** → 방금 만든 저장소 선택.
3. Netlify가 Next.js 프로젝트임을 자동으로 인식합니다. 빌드 명령어(`next build`)와 배포 설정은 기본값 그대로 두면 됩니다.
4. **Site configuration → Environment variables**에서 아래 두 개를 추가:
   - `NOTION_API_KEY`
   - `NOTION_DATABASE_ID`
5. **Deploy site** 클릭. 몇 분 뒤 `https://프로젝트이름.netlify.app` 주소가 생깁니다.
6. 이후로는 노션에 새 글을 쓰고 Published를 체크하기만 하면, 최대 30초 이내에 사이트에 자동으로 반영됩니다(ISR). 재배포할 필요 없습니다.

Netlify는 Next.js의 ISR(자동 갱신)을 기본 어댑터에서 그대로 지원하므로, 별도 플러그인 설정 없이도 `revalidate: 30` 설정이 정상 동작합니다.
...
Netlify 프로젝트 → **Site configuration → Domain management**에서 갖고 계신 도메인을 추가하고, 안내되는 DNS 레코드를 도메인 구매처(가비아, 후이즈 등)에 등록하면 됩니다.
